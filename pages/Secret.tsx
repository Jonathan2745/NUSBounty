  ...
  const changeBalance = async (amount: number) => {
    ...
    if (user && currentUser) {
      ...
      try {
        ...
        console.log("balance Changed", { amount }, "current Balance", {
          currentBalance,
        });
      } catch (error) {
    ...
    <MainTemplate currentNavigation={"wallet"}>
      <div className="flex flex-col items-center justify-center min-h-screen">
        ...
        <h1 className="text-5xl mb-6 font-semibold">
          This Page is inteded for the testing and bug fixing of wallet related
          modules/ features
        </h1>

        <Button onClick={() => changeBalance(1234567)}>
          {" "}
          Test Balance 1234567{" "}
        </Button>
        <ul className="text-white flex"> Wallet Balance: {balance}</ul>
        {/* // make this button add 2$ to the current users wallet // */}
        <Button loadingText="" onClick={() => changeBalance(2)}>
          {" "}
          Give me $2{" "}
        </Button>
